...
void mu2000::build_bus()
{
	...
	auto swp = [this](swp30_device &dev, u32 base) {
		// 実機の SWP30 は、書き込みを 1 サンプルに 1 回しか受け取れないらしく、書くたびに CPU は次の
		// サンプルの区切りまで待たされる（BSC の WAIT）。遅れて鳴る層（firmware が 2.5ms ごとに数を減らして
		// 鍵を押す）の遅れが、これで実機と音ごとに ±10 サンプルで合う。待たせないと、音の設定の書き込み
		// 百回ほどが一瞬で終わり、実機より約 64 サンプル長くなっていた。制御の 2 つ（0x0e / 0x0f：
		// 鍵を押す合図、MEG のプログラムの番地と中身、波形やリバーブ RAM の直の読み書き）は待たせない
		auto hold = [this](offs_t reg) {
			const u32 slot = reg & 0x3f;
			if (slot != 0x0e && slot != 0x0f) {
				m_swp_hold = true;
