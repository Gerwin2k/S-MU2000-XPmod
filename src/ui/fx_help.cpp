...
// オーナーズマニュアルのエフェクトタイプリストとパラメーター解説を読んで確かめた
// （文は写していない）。EX で足された種類（T.DELAY、DYNA FLT、SLICE など）はそこに無いので、
// 名前と、エミュレータの LCD に出るパラメータの並びから書いた。
...
const type_text TYPES[] = {
	...
	{ 0x15, 0x00, "テンポに合わせた長さで繰り返すディレイ（EX で追加）", "Tempo-synced delay (added in the EX)." },
	{ 0x15, 0x08, "テンポに合わせた長さで繰り返すエコー（EX で追加）", "Tempo-synced echo (added in the EX)." },
	{ 0x16, -1, "テンポに合わせた長さで、左右で交差するディレイ（EX で追加）", "Tempo-synced cross delay (added in the EX)." },
	...
	{ 0x68, -1, "ビンテージのフランジャーの真似（EX で追加）", "Vintage flanger (added in the EX)." },
	{ 0x6b, -1, "テンポに合わせて揺れるフランジャー（EX で追加）", "Tempo-synced flanger (added in the EX)." },
	{ 0x6c, -1, "テンポに合わせて揺れるフェイザー（EX で追加）", "Tempo-synced phaser (added in the EX)." },
	{ 0x6e, -1, "入ってくる音の大きさで動くフランジャー（EX で追加）", "Dynamic flanger, driven by the input level (added in the EX)." },
	{ 0x6f, -1, "入ってくる音の大きさで動くフェイザー（EX で追加）", "Dynamic phaser, driven by the input level (added in the EX)." },
	...
	{ 0x4b, 0x01, "ギターアンプの真似（もう 1 つの型）", "Guitar amp simulator, second model." },
	...
	{ 0x6d, -1, "入ってくる音の大きさで動くフィルタ（EX で追加）", "Dynamic filter, driven by the input level (added in the EX)." },
	{ 0x74, -1, "低域を強調して共鳴させるフィルタ（EX で追加）", "Low-frequency resonance filter (added in the EX)." },
	{ 0x73, -1, "低・中・高の帯を切ったり残したりする。DJ ミキサーのアイソレーター（EX で追加）", "Isolator: cut or keep the low, mid and high bands like a DJ mixer (added in the EX)." },
	...
	{ 0x69, -1, "帯ごとに別々に掛かるコンプレッサー（EX で追加）", "Multi-band compressor (added in the EX)." },
	...
	{ 0x70, -1, "入ってくる音の大きさで動くリングモジュレーター（EX で追加）", "Dynamic ring modulator, driven by the input level (added in the EX)." },
	{ 0x71, -1, "別の波を掛け合わせて、金属的な音にする（リングモジュレーター。EX で追加）", "Ring modulator: multiplies with another wave for a metallic sound (added in the EX)." },
	{ 0x72, -1, "テンポに合わせて音を細かく切り刻む（EX で追加）", "Slice: chops the sound in time with the tempo (added in the EX)." },
	{ 0x75, -1, "レコードプレーヤーの針の雑音や回転の揺れの真似（EX で追加）", "Digital turntable: record noise and platter wobble (added in the EX)." },
	{ 0x76, -1, "レコードをこするスクラッチの真似（EX で追加）", "Digital scratch: imitates record scratching (added in the EX)." },
};
...
const param_text PARAMS[] = {
	...
	{ "Dly L>R", "左から右へのディレイの長さ", "Delay time from left to right." },
	{ "Dly R>L", "右から左へのディレイの長さ", "Delay time from right to left." },
	{ "InputSelect", "どちらのチャンネルの音を入れるか", "Which channel feeds the effect." },
	{ "DelayTime", "ディレイの長さ（カラオケでは反射の間隔）", "Delay time (for karaoke, the spacing of the echoes)." },
	...
	{ "LFO Wave", "揺れの波の形", "Waveform of the modulation." },
	...
	{ "Mod Mix", "揺れた音を混ぜる量", "How much modulated sound is mixed in." },
	{ "ModPhase", "揺れの位相", "Modulation phase." },
	...
	{ "AnalogFeel", "アナログ機器らしい癖の量", "Amount of analogue character." },
	// EQ・フィルタ
	...
	{ "Sensitivty", "入ってくる音の変化にフィルタがどれだけ反応するか", "How strongly the filter reacts to changes in the input." },
	{ "FltrType", "フィルタの種類", "Filter type." },
	{ "Freq Fine", "周波数の細かい調整", "Fine frequency adjustment." },
	{ "FreqCourse", "周波数の大まかな調整", "Coarse frequency adjustment." },
	// ダイナミクス
	{ "Attack", "効き始めるまでの時間（ゲートでは開き始めるまで）", "Time until the effect kicks in (for a gate, until it opens)." },
	{ "AttackTime", "効き始めるまでの時間", "Attack time." },
	...
	{ "RelesTime", "効き終わるまでの時間", "Release time." },
	{ "RelesCurve", "効き終わり方のカーブ", "Shape of the release." },
	{ "Threshold", "効き始める入力の大きさ（ゲートでは開く大きさ）", "Input level at which it acts (for a gate, the opening level)." },
	{ "ThreshLevel", "効き始める入力の大きさ", "Threshold level." },
	{ "ThreshOfst", "効き始める大きさの基準", "Threshold offset." },
	{ "Ratio", "コンプレッサーの圧縮の比率", "Compression ratio." },
	{ "Gate Time", "ゲートが開いている時間", "How long the gate stays open." },
	{ "LevelOfst", "音量の基準", "Level offset." },
	{ "Lag", "反応の遅れ", "Response lag." },
	// ピッチ・その他
	...
	{ "PhasInv", "位相を反転する", "Inverts the phase." },
	...
	{ "Resoltn", "細かさ（分解能）", "Resolution." },
	{ "Direction", "向き", "Direction." },
	{ "Reset", "揺れを始めからにする", "Restarts the modulation." },
	{ "On/Off", "入り切り", "On or off." },
	{ "Aeg Phase", "音量の動きの位相", "Phase of the amplitude envelope." },
	{ "PanAegType", "位置と音量の動きの型", "Type of pan/amplitude pattern." },
	{ "PanAegLvl", "位置と音量の動きの深さ", "Depth of the pan/amplitude pattern." },
	{ "DivideType", "刻み方の型", "Slicing pattern." },
	{ "DivideLvl", "刻む深さ", "Slicing depth." },
	{ "ScratchSpd", "スクラッチの速さ", "Scratch speed." },
	{ "ScratchDpth", "スクラッチの深さ", "Scratch depth." },
	{ "DryToNoise", "元の音と雑音の釣り合い", "Balance between the dry sound and noise." },
	...
	{ "L/R Diff", "左右の差", "Difference between left and right." },
	// 共通
