...
constexpr int XG_PART_COPY = 0xe0;      // xg::ram::PART_COPY
constexpr int XG_SYSTEM_SIZE = 7;
constexpr int XG_EFFECT_SIZE = 0xfe;    // xg::ram::EFFECT_SIZE

struct xg_snapshot {
	u64 serial = 0;                                  // 写すたびに増える
	u8  system[XG_SYSTEM_SIZE] = {};
	u8  effect[XG_EFFECT_SIZE] = {};                 // 02 01 00 からインサーション 2 まで（RAM の並び）
