...
| | |
|---|---|
| 待ち時間 | 70ms（1024 サンプル × 3 枚）。`--frames` `--buffers` で調整 |
| CPU 使用率 | 60% 前後 |
| 詰めるなら | waveOut を WASAPI 共有モードに差し替える。触るのは `live.cpp` だけ |

**waveOut の 1 枚は 1024 サンプル(23ms)を割ってはいけない。**
この環境の waveOut は 20ms 前後の間隔でしか枚を回収しない。それより短い枚に
すると供給が追いつかず、再生が遅れて細切れに聞こえる。20 秒ぶんの音を
作らせて 1 枚あたりの実時間を割り出した実測:
...
| 256 (5.8ms) | 12.7ms |
| 512 (11.6ms) | 14.5ms |
| 1024 (23.2ms) | 21.7ms ← ここで正常 |

timeBeginPeriod(1) を呼んでも変わらない。**待ち時間を詰めたいなら
waveOut をやめて WASAPI の共有モードにするしかない。**

### 速さの内訳（実測）

音 37.9 秒を作るのに 24 秒。内訳は

| | |
|---|---|
| CPU と内蔵周辺 | 10.0 秒 (42%) |
| SWP30 の MEG（エフェクト DSP） | 12.2 秒 (51%) |
| SWP30 の AWM2 とミキサ | 1.4 秒 (6%) |

**MEG が一番重い。** 1 サンプルにつき 384 ステップ、2 個で毎秒 3390 万ステップ。
MAME はここを DRC で回している。こちらはインタプリタなので、詰めるなら
命令をあらかじめ解いた形に落とすか、DRC を書くことになる。

命令とリバーブ RAM をアドレス空間ごしでなく配列から直に読むようにして 12%
速くなった（出力はビット単位で変わらないことを確認済み）。

タイマの「次に鳴る時刻」を覚える形も試したが、**速くならないうえに出力が
変わった**ので捨てた。8 本を数え直す方が安い。
