...
| | 待ち時間 | 備考 |
|---|---|---|
| WASAPI 共有モード（既定） | 30ms（`--latency` で調整） | イベント駆動 |
| WinMM waveOut (`--waveout`) | 70ms | 1 枚 23ms を割ると供給が追いつかない |

**待ち時間の下限は音源の速さで決まる。** Windows の最小周期（この環境で
23.5ms）だと音源の山で 25 秒に 1 回ほどデバイスの残量がゼロになる。
30ms なら 0 回。これ以上詰めたいなら音を作るのを速くするしかない。
