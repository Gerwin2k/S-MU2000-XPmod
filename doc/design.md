...
段階 4 の作り。音声デバイスが空けた分だけ `run_sample()` を回す。
**自分で時計を持たない**のがすべてで、ここが MAME と決定的に違う。
MIDI 入力はコールバックが別スレッドで来るので、錠を使わない輪
（single producer / single consumer）でバイトを音声スレッドへ渡す。
音声スレッドで錠を待たせてはいけない。
...
| | 待ち時間 | 備考 |
|---|---|---|
| WASAPI 共有モード（既定） | 45ms（`--latency` で調整） | イベント駆動 |
| WinMM waveOut (`--waveout`) | 70ms | 1 枚 23ms を割ると供給が追いつかない |

**待ち時間の下限は音源の速さで決まる。** WASAPI に Windows の最小周期
（この環境で 23.5ms）を要求すると、音源の山で間に合わず 25 秒に 1 回ほど
デバイスの残量がゼロになる（耳で分かる）。45ms あれば枯渇しない。
これ以上詰めたいなら、音を作るのを速くするしかない。

**枯渇の判定は生成時間ではなく GetCurrentPadding。** 1 回の生成が何 ms
かかったかを溜めの長さと比べても、実際に音が切れたかは分からない。
イベントが来た直後の残量がゼロなら、デバイスは鳴らし終えて待たされている。
これが本当の音切れ。
