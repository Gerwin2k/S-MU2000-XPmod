...
待ち時間は **`--latency` が「溜める目標」**（既定 20ms）。溜めは満杯にせず
目標までしか書かないので、その値がほぼそのまま遅れになる。実測は画面と
終了時の報告に出る。

**`--exclusive` を付けるとデバイスを独り占めして Windows の混ぜ合わせを
通らない。** この機械では出るまで約 10ms（共有だと 10-20ms + 混ぜ合わせの分）。
代わりに、鳴らしている間は他のアプリが音を出せない。

| | 出るまで | 余裕 |
|---|---|---|
| 共有 `--latency 20`（既定） | 約 10-20ms | 20ms |
| 共有 `--latency 30` | 約 20-30ms | 30ms |
| `--exclusive --latency 10` | 約 10ms | 10ms |
